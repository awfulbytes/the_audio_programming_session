#include <stdio.h>
#include <stdlib.h>

int mod12(int note) {
    while (note < 0) note += 12;
    while (note >= 12) note -= 12;
    return note;
}

int main(int argc, char** argv) {
    int series[12][12], offset;
    int n, m, i;
    char* table[12] = {"C", "Db", "D", "Eb",
                       "E", "F", "Gb", "G",
                       "Ab", "A", "Bb", "B" };

    if (argc != 13) {
        printf("usage: %s note1 note2 .. note12\n", argv[0]);
        return -1;
    }

    // loop until you get 12 notes!!
    for(n = 0; n < 12; n++)
        series[0][n] = mod12(atoi(argv[n+1]));

    // create inversion in col 1
    for(m = 1; m < 12; m++)
        series[m][0] = mod12(series[m-1][0] + series[0][m-1] - series[0][m] - 1);

    // create all transpositions
    for (m = 1; m < 12; m++)
        for (n = 1; n < 12; n++)
            series[m][n] = mod12(series[0][n] + series[m][0] - series[0][0] - 1);

    for (m = 0; m < 12; m++){
        for (n = 0; n < 12; n++) printf("%s\t", table[series[m][n]]);
        printf("\n");
    }
    return 0;
}
